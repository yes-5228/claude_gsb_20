...
| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布（各状态列含超期数）、区域运行情况、重点关注公厕、最新问题与巡查 |
...
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、按分类/程度自动推算整改期限、整改流程流转、整改轨迹时间线、超期与剩余天数标记、追加跟进记录、人工调整期限需填原因 |
| 系统设置 | `/settings` | 维护「分类 × 严重程度」期限规则矩阵（自然日/工作日）、按年份维护法定节假日与调休补班 |
...
| POST | `/issues` | 上报问题，自动生成编号 `WT-YYYYMMDD-001` 并写入首条整改流水；期限留空按规则自动推算 |
| GET/PATCH/DELETE | `/issues/{id}` | 详情（含完整整改轨迹）/ 更新 / 删除；PATCH 调整期限必须带 `deadline_adjust_reason` |
| GET | `/issues/{id}/transitions` | 当前状态可执行的流转动作 |
| POST | `/issues/{id}/transitions` | 推进整改状态（越级流转返回 400）；进入闭环时冻结超期结论 |
| POST | `/issues/{id}/records` | 追加跟进记录（不改变状态） |
| GET/PUT | `/deadline/rules` | 期限规则矩阵（分类×程度 → 天数 + 自然日/工作日） |
| GET | `/deadline/preview` | 按分类与程度推算建议期限（上报表单实时预览） |
| GET/PUT | `/calendar/holidays?year=` | 按年份查询 / 整体保存节假日与调休补班 |
...
- **期限自动推算**：上报时期限默认由系统按「问题分类 × 严重程度」推算，规则可在系统设置中维护；支持按**自然日**或**工作日**计，工作日自动跳过周末与法定节假日、遇调休补班日照常计入；**紧急问题一律当天到期**（截止当天 23:59，临近下班才上报也不会一上报即超期）。手填/修改期限若与系统建议不一致，**必须填写调整原因**，并写入整改轨迹（动作为「调整期限」）。
- **节假日日历**：按年份维护，只需登记与普通周末口径不同的日期——法定放假日（休息）与调休补班日（上班）。按年份整体保存，只影响之后新推算的期限。
- **超期判定与统一口径**：超期/今日到期/剩余天数全部由后端统一计算（`is_overdue`、`due_today`、`days_remaining`、`overdue_days`），列表、详情、工作台展示的是同一份结论；前端在页面可见时每 30~60 秒自动重取，期限临近或被调整后实时更新。看板各状态列单独显示其中的超期数量。
- **超期结论冻结**：问题进入「已完成/已关闭」时把当时是否超期、超期天数定格到快照；此后跨年调整节假日日历或规则，**不会反过来改写**已闭环问题的历史超期结论（未闭环问题仍按当前日历实时判定）。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、多条不同整改阶段的问题及其完整整改轨迹、18 条期限规则与 2024—2026 年节假日日历（含调休补班），其中含超期、今日到期与人工调整期限的样例。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
...
- 后端 `pytest`：15 个用例全部通过；`/health`、台账/巡查/问题/统计/字典、期限规则与节假日接口均返回预期数据。
- 前端 `npm run build`：构建成功（72 个模块）。
