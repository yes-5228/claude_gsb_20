...
| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡、巡查与问题趋势、整改状态/分类/严重程度分布（含各状态列超期数量）、区域运行情况、重点关注公厕、最新问题与巡查 |
...
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、分类/程度/期限、期限按规则自动推算、人工调整留痕、整改流程流转、整改轨迹时间线、超期预警、追加跟进记录 |
| 节假日日历 | `/holidays` | 按年份维护法定节假日，工作日推算期限时自动跳过 |
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / issues / holidays / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量（含期限推算规则矩阵）、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、节假日
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、期限推算、节假日、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / issues / holidays 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| POST | `/issues` | 上报问题，自动生成编号 `WT-YYYYMMDD-001` 并写入首条整改流水；未指定期限时按规则自动推算 |
| GET | `/issues/deadline-preview` | 按分类与严重程度预览推算出的整改期限 |
...
| GET | `/holidays?year=` | 查询某一年的节假日日历 |
| PUT | `/holidays/{year}` | 整体维护某一年的节假日（日期必须属于该年） |
| DELETE | `/holidays/{year}` | 清空某一年的节假日 |
...
- **整改期限自动推算**：上报时不填期限则按「分类 × 严重程度」规则自动计算，紧急问题一律当天到期（当天 23:59）；自然日直接顺延，工作日跳过周末与节假日日历中的日期。规则矩阵：

  | 分类 | 一般 | 严重 | 紧急 |
  | --- | --- | --- | --- |
  | 保洁不到位 / 耗材缺失 | 2 个自然日 | 1 个自然日 | 当天 |
  | 异味扰民 | 3 个自然日 | 2 个自然日 | 当天 |
  | 设施损坏 | 5 个工作日 | 3 个工作日 | 当天 |
  | 安全隐患 | 3 个自然日 | 1 个自然日 | 当天 |
  | 其他 | 5 个自然日 | 3 个自然日 | 当天 |

  期限仍是「自动推算」时，修改分类或严重程度会按规则重算；人工调整过的期限不受影响，可通过「按规则重新推算」恢复。
- **人工调整留痕**：通过编辑接口修改期限必须填写 `deadline_reason`，系统把「调整期限 / 重算期限」写入整改轨迹（原期限 → 新期限、原因、操作人），详情页时间线可追溯。
- **超期判定单一口径**：超期与剩余天数由服务端实时计算并随接口返回（`is_overdue`、`remaining_days`），列表、详情、看板展示完全一致；期限或状态一变，看板各状态列的超期数量与列表标记同步更新。
- **节假日日历**：按年份维护（`PUT /holidays/{year}` 整体替换），仅影响保存之后新推算的期限；已有问题的期限与超期结论保持不变，跨年调整不会回溯改写历史。
...
`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹（期限按规则自动推算，部分未闭环问题已超期，便于演示超期预警），以及当年与次年的节假日日历。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
...
- 后端 `pytest`：17 个用例全部通过，覆盖台账 CRUD、删除保护、评分、流程流转、统计，以及期限推算（自然日/工作日/紧急当天/节假日跳过）、人工调整原因校验、自动重算、节假日按年维护与跨年不回溯。
- 前端 `npm run build`：构建成功（70 个模块）。
- 浏览器端到端验证（Chromium 无头模式，覆盖 6 个场景 25 项断言）：看板各状态列超期数量、问题列表剩余期限、上报表单自动推算预览（紧急当天到期、设施损坏 5 个工作日）、人工调整期限的原因拦截与整改轨迹留痕、恢复自动推算、节假日按年增删保存、列表/详情/看板剩余天数单一口径。全程无控制台报错。
